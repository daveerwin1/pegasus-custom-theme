...
FocusScope {
    property bool prevPageHeld: false
    property bool nextPageHeld: false
    ...
    Keys.onPressed: (event) => {
        ...
        if (api.keys.isPrevPage(event)) {
            ...
            if (event.isAutoRepeat)
                ...
                if (!prevPageHeld) {
                    prevPageHeld = true
                    topbar.prev()        // fire immediately
                    repeatTimer.start()  // start repeat loop
                }
                return
        }
