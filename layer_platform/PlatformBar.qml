...
Rectangle {
    ...
    PathView {
        ...
        path: Path {
            startX: vpx((parent.parent.width * 0.3) / 2)
            startY: vpx(36)
            PathAttribute { name: "itemZ"; value: 201 }

            PathLine { x: parent.width * 0.13; y: vpx(36) }
            PathPercent { value: 0.00 }
            PathAttribute { name: "itemZ"; value: 200 }

            PathLine { x: parent.width * 0.15; y: vpx(18) }
            PathPercent { value: 0.02 }
            PathAttribute { name: "itemZ"; value: 199 }

            PathLine { x: parent.width * 0.9; y: vpx(18) }
            PathPercent { value: 1 }
        ...
        preferredHighlightBegin: 0.00
        preferredHighlightEnd: 0.01
    }
    ...
    Component {
        ...
        PlatformCard {
            platformShortName: model.shortName
            platformCardLabel: model.extra.shortname_display === undefined ? platformShortName : model.extra.shortname_display
